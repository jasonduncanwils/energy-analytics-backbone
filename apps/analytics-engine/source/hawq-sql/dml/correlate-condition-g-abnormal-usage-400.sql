...
FROM   ( 
       ...
       (
        ...
        WHERE 
         CM.correlation_code = 'G_ABNORMAL_USAGE_400'
       ) TMP2, 
       grid.component               CP 
WHERE 
...
EXECUTE correlation_instance('USAGE_DEVIATION_AFTER_METER_CHANGE',
                             grid.getCorrelationThreshold('G_ABNORMAL_USAGE_400',
                                                          'CONDITION_CODE',
                                                          'USAGE_DEVIATION_AFTER_METER_CHANGE')
                            );
