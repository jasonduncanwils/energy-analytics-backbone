...
FROM   ( 
       ...
       (
        ...
        WHERE 
         CM.correlation_code = 'E_COMMUNICATION_ISSUE_200'
       ) TMP2, 
       grid.component               CP 
WHERE 
...
EXECUTE correlation_instance('AMI_COMM_LOSS_AFTER_REMOTE_DISCONNECT',
                             grid.getCorrelationThreshold('E_COMMUNICATION_ISSUE_200',
                                                          'CONDITION_CODE',
                                                          'AMI_COMM_LOSS_AFTER_REMOTE_DISCONNECT')
                            );
