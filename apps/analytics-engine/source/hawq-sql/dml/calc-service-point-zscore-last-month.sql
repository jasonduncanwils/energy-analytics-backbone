...
FROM 
        ...
        SELECT 
         ...
         THEN 
              (
               ...
               - 
               MAX(CA2.calc_value) -- AVG_KW_30_DAY_ROLLING from last month 
              ) 
              / 
              MAX(CA3.calc_value) -- STDDEV_30_DAY_ROLLING from last month 
         -- ********************************** 
         WHEN TMP2.measurement_type IN ('KWH-DEL','KWH-REC') 
          AND CA3.calc_value = 0 
         THEN 0 
        ...
        FROM 
                ...
                WHERE 
                 ...
                 AND SC.Calc_Function = 'Z_SCORE_VS_LAST_MONTH' 
                ORDER BY 
                 SC.UID_Group, 
                 SC.End_Date_Top, 
                 SC.Calc_Sequence 
        ...
        INNER JOIN  grid.vw_calc                    CA3 
         ...
         AND CA3.End_Date = TMP2.End_Date_Top 
