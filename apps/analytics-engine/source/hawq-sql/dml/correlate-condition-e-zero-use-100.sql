...
FROM   ( 
       ...
       (
        ...
        WHERE 
         CM.correlation_code = 'E_ZERO_USE_100'
       ) TMP2, 
       grid.component               CP 
WHERE 
...
EXECUTE correlation_instance('ZERO_USAGE_AFTER_METER_CHANGE',
                             grid.getCorrelationThreshold('E_ZERO_USE_100',
                                                          'CONDITION_CODE',
                                                          'ZERO_USAGE_AFTER_METER_CHANGE')
                            );
